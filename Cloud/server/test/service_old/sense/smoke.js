...
exports.create = function (sensorId,value) {
    ...
    Sensor.findOne({_id: sensorId}, function (err, result) {
        if (err) {
            ...
            Smoke.findOneAndUpdate(queryCrite, insertCrite, {upsert: true, new: true}, function (err, smoke) {
                if (err) {
                    ...
                    console.log('创建烟雾数据成功');
                    return  '创建烟雾数据成功'
                }else{
                    log4j.error('创建烟雾数据失败2');
                    return  '创建烟雾数据失败2'
                }
            });
        }
...
function __dbaggregate(criteria, timeSeries, cb) {
    ...
    //console.log(o)
    Smoke.mapReduce(o, function (err, model, stats) {
        if (err || !model) {
