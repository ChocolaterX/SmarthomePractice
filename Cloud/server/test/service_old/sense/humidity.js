...
exports.create = function (sensorId,value) {
    ...
    Sensor.findOne({_id: sensorId}, function (err, result) {
        if (err) {
            ...
            Humidity.findOneAndUpdate(queryCrite, insertCrite, {upsert: true, new: true}, function (err, humidity) {
                if (err) {
                    ...
                    console.log('创建湿度数据成功');
                    return  '创建湿度数据成功'
                }else{
                    log4j.error('创建湿度数据失败2');
                    return  '创建湿度数据失败2'
                }
            });
        }
...
function __dbaggregate(criteria, timeSeries, cb) {
    ...
    //console.log(o)
    Humidity.mapReduce(o, function (err, model, stats) {
        if (err || !model) {
