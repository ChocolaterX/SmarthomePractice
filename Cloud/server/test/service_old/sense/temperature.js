...
exports.create = function (sensorId,value) {
    ...
    Sensor.findOne({_id: sensorId}, function (err, result) {
        if (err) {
            ...
            Temperature.findOneAndUpdate(queryCrite, insertCrite, {upsert: true, new: true}, function (err, temperature) {
                if (err) {
                    ...
                    console.log('创建温度数据成功');
                    return  '创建温度数据成功'
                }else{
                    log4j.error('创建温度数据失败2');
                    return  '创建温度数据失败2'
                }
            });
        }
...
function __dbaggregate(criteria, timeSeries, cb) {
    ...
    //console.log(o)
    Temperature.mapReduce(o, function (err, model, stats) {
        if (err || !model) {
